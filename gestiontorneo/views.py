...
@login_required
def asignar_llaves(request, torneo_id):
    torneo = get_object_or_404(Torneo, id=torneo_id, organizador=request.user)
    
    if not torneo.inscripciones_cerradas:
        messages.error(request, "Debes cerrar las inscripciones antes de asignar las llaves.")
        return redirect('gestionar_torneo', torneo_id=torneo.id)
    
    participantes = list(torneo.participaciones.select_related('jugador').all())
    num_participantes = len(participantes)
    jugadores = [p.jugador for p in participantes]
    
    # Calcular la potencia de 2 más cercana (hacia arriba)
    potencia_2_siguiente = 2 ** math.ceil(math.log2(max(num_participantes, 2)))
    
    if request.method == "POST":
        # Limpiar llaves y BYEs existentes
        LlaveTorneo.objects.filter(torneo=torneo).delete()
        JugadorBYE.objects.filter(torneo=torneo).delete()
        
        asignaciones = []
        asignados_ids = set()
        byes_marcados = set()
        
        # Recoger BYEs marcados manualmente
        for i in range(potencia_2_siguiente):
            bye_marcado = request.POST.get(f'bye_{i}')
            if bye_marcado == 'on':  # Checkbox marcado
                byes_marcados.add(i)
        
        # Recoger asignaciones manuales de jugadores
        for i in range(potencia_2_siguiente):
            if i in byes_marcados:
                # Posición marcada como BYE
                asignaciones.append(('bye', None))
            else:
                jugador_id = request.POST.get(f'casilla_{i}')
                if jugador_id and jugador_id not in ("random", "bye"):
                    try:
                        jugador_id = int(jugador_id)
                        if jugador_id not in asignados_ids:
                            # Verificar que el jugador esté en la lista de participantes
                            if any(j.id == jugador_id for j in jugadores):
                                asignados_ids.add(jugador_id)
                                asignaciones.append(('jugador', jugador_id))
                            else:
                                asignaciones.append(('bye', None))  # Jugador no válido -> BYE
                        else:
                            asignaciones.append(('bye', None))  # Jugador duplicado -> BYE
                    except ValueError:
                        asignaciones.append(('random', None))
                else:
                    asignaciones.append(('random', None))

        # Asignar aleatoriamente los no asignados
        no_asignados = [j.id for j in jugadores if j.id not in asignados_ids]
        random.shuffle(no_asignados)
        
        # Calcular cuántos BYEs necesitamos
        byes_necesarios = potencia_2_siguiente - num_participantes
        byes_creados = 0
        
        for idx, (tipo, valor) in enumerate(asignaciones):
            if tipo == 'random':
                if no_asignados and byes_creados < byes_necesarios:
                    # Si aún hay jugadores y no hemos creado todos los BYEs necesarios
                    # decidir si asignar jugador o BYE basado en lo que falta
                    jugadores_restantes = len(no_asignados)
                    posiciones_restantes = len([x for x in asignaciones[idx:] if x[0] == 'random'])
                    byes_restantes_necesarios = byes_necesarios - byes_creados
                    
                    if posiciones_restantes <= byes_restantes_necesarios:
                        # Todas las posiciones restantes deben ser BYEs
                        bye = JugadorBYE.objects.create(
                            torneo=torneo,
                            posicion_bye=byes_creados + 1
                        )
                        asignaciones[idx] = ('bye', bye.id)
                        byes_creados += 1
                    else:
                        # Asignar jugador
                        asignaciones[idx] = ('jugador', no_asignados.pop())
                elif no_asignados:
                    # Asignar jugador
                    asignaciones[idx] = ('jugador', no_asignados.pop())
                else:
                    # No hay más jugadores, crear BYE si es necesario
                    if byes_creados < byes_necesarios:
                        bye = JugadorBYE.objects.create(
                            torneo=torneo,
                            posicion_bye=byes_creados + 1
                        )
                        asignaciones[idx] = ('bye', bye.id)
                        byes_creados += 1
            elif tipo == 'bye' and valor is None:
                # Crear BYE para posiciones marcadas manualmente
                bye = JugadorBYE.objects.create(
                    torneo=torneo,
                    posicion_bye=byes_creados + 1
                )
        ...
        # Crear las llaves de la primera ronda
        primera_ronda = 1
        partidos_primera_ronda = potencia_2_siguiente // 2
        
        for i in range(partidos_primera_ronda):
            pos1 = i * 2
            pos2 = i * 2 + 1
            ...
            if pos1 < len(asignaciones):
                tipo1, id1 = asignaciones[pos1]
                if tipo1 == 'jugador':
                    llave.jugador1 = Jugador.objects.get(id=id1)
                elif tipo1 == 'bye':
                    llave.bye1 = JugadorBYE.objects.get(id=id1)
            
            # Asignar participante 2
            if pos2 < len(asignaciones):
                tipo2, id2 = asignaciones[pos2]
                if tipo2 == 'jugador':
                    llave.jugador2 = Jugador.objects.get(id=id2)
                elif tipo2 == 'bye':
                    llave.bye2 = JugadorBYE.objects.get(id=id2)
            
            llave.save()
            
            # Si hay un BYE, avanzar automáticamente al jugador
            ganador_automatico = llave.avanzar_automaticamente()
            if ganador_automatico:
                llave.save()

        # Crear llaves vacías para rondas siguientes
        rondas_necesarias = math.ceil(math.log2(potencia_2_siguiente))
        for ronda in range(2, rondas_necesarias + 1):
            partidos_ronda = potencia_2_siguiente // (2 ** ronda)
            for pos in range(1, partidos_ronda + 1):
        ...
        # Marcar torneo como iniciado con modalidad llaves
        torneo.modalidad = 'llaves'
        torneo.torneo_iniciado = True
        torneo.save()

        # Crear mensaje de éxito detallado
        asignados_manualmente = len(asignados_ids)
        byes_totales = byes_creados
        
        mensaje = f"Llaves creadas exitosamente. "
        mensaje += f"Jugadores asignados manualmente: {asignados_manualmente}, "
        mensaje += f"BYEs creados: {byes_totales}."
        
        messages.success(request, mensaje)
        return redirect('gestionar_llaves', torneo_id=torneo.id)

    # Calcular información adicional para el contexto
    byes_necesarios = potencia_2_siguiente - num_participantes
    partidos_primera_ronda = potencia_2_siguiente // 2
    ...
    context = {
        ...
        'num_participantes': num_participantes,
        'potencia_2_siguiente': potencia_2_siguiente,
        'byes_necesarios': byes_necesarios,
        'partidos_primera_ronda': partidos_primera_ronda,
        'casillas': list(range(potencia_2_siguiente)),
    }
    return render(request, 'asignar_llaves_profesional.html', context)

@login_required
def gestionar_llaves(request, torneo_id):
    torneo = get_object_or_404(Torneo, id=torneo_id, organizador=request.user)
    
    if torneo.modalidad != 'llaves':
        messages.error(request, "Este torneo no está configurado en modalidad llaves.")
        return redirect('gestionar_torneo', torneo_id=torneo.id)
    
    # Procesar avances automáticos de BYEs
    llaves_con_bye = LlaveTorneo.objects.filter(
        torneo=torneo, 
    ...
    for llave in llaves_con_bye:
        ganador_automatico = llave.avanzar_automaticamente()
        if ganador_automatico:
            llave.save()
            # Avanzar a la siguiente ronda
            siguiente_ronda = llave.ronda + 1
            siguiente_posicion = (llave.posicion + 1) // 2
            
            try:
                siguiente_llave = LlaveTorneo.objects.get(
                    torneo=torneo,
                    ronda=siguiente_ronda,
                    posicion=siguiente_posicion
                )
                
                if llave.posicion % 2 == 1:  # Posición impar -> jugador1
                    siguiente_llave.jugador1 = ganador_automatico
                else:  # Posición par -> jugador2
                    siguiente_llave.jugador2 = ganador_automatico
                
                siguiente_llave.save()
                
            except LlaveTorneo.DoesNotExist:
                pass  # Es la final
    
    # Obtener todas las llaves organizadas por ronda
    llaves_por_ronda = {}
    for llave in LlaveTorneo.objects.filter(torneo=torneo).order_by('ronda', 'posicion'):
        if llave.ronda not in llaves_por_ronda:
            llaves_por_ronda[llave.ronda] = []
        llaves_por_ronda[llave.ronda].append(llave)
    
    # Procesar resultados si se envía el formulario
    if request.method == "POST":
        llave_id = request.POST.get('llave_id')
        ganador_id = request.POST.get('ganador_id')
        
        if llave_id and ganador_id:
            llave = get_object_or_404(LlaveTorneo, id=llave_id, torneo=torneo)
            ganador = get_object_or_404(Jugador, id=ganador_id)
            
            # Validar que el ganador sea uno de los jugadores de la llave
            if ganador in [llave.jugador1, llave.jugador2]:
                llave.ganador = ganador
                llave.partido_jugado = True
                llave.save()
                ...
                try:
                    ...
                    # Asignar el ganador a la siguiente llave
                    if llave.posicion % 2 == 1:  # Posición impar -> jugador1
                        siguiente_llave.jugador1 = ganador
                    else:  # Posición par -> jugador2
                        siguiente_llave.jugador2 = ganador
                    
                    siguiente_llave.save()
                    
                except LlaveTorneo.DoesNotExist:
                    # Es la final, no hay siguiente ronda
                    messages.success(request, f"¡{ganador.nombre} {ganador.apellido} es el campeón del torneo!")
                
                messages.success(request, f"Resultado guardado. {ganador.nombre} {ganador.apellido} avanza a la siguiente ronda.")
            else:
                messages.error(request, "El ganador seleccionado no participa en este partido.")
        
        return redirect('gestionar_llaves', torneo_id=torneo.id)
    
    context = {
        'torneo': torneo,
        'llaves_por_ronda': llaves_por_ronda,
        'rondas': sorted(llaves_por_ronda.keys()) if llaves_por_ronda else [],
    }
    
    return render(request, 'gestionar_llaves.html', context)
